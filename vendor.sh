...
function download_grammar() {
    ...
    url="https://mirror.ghproxy.com/https://raw.githubusercontent.com/$repository"
    mkdir -p "$target"

    echo "downloading $lang $version"
    echo "$url/$version/src/tree_sitter/parser.h"
    echo "$target/parser.h"
    curl -s -f -S "$url/$version/src/tree_sitter/parser.h" -o "$target/parser.h"
    echo "$files"
    for file in $files; do
        echo "downloading $file"
        echo "$url/$version/src/$file"
        echo "$target/$file"
